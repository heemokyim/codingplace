...
def project_along(b, v, eps = 1E-20):
    ...
    # 2017.8.13
    # inner product on complex-field
    # also applied to rational-field

    v_norm = sum(val.real ** 2 + val.imag ** 2 for val in v.f.values())
    sigma = (b*v)/v_norm if v_norm > eps else 0
    return sigma * v
...
def project_orthogonal(b, vlist):
    ...
    for v in vlist:
        bhat=Vec(b.D,{d:val.conjugate() for d,val in b.f.items()})
        b = b - project_along(bhat, v)
