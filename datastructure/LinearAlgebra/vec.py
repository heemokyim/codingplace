...
def add(u, v):
    """
        ...
        """
    assert u.D == v.D
    return Vec(u.D,{k:u[k]+v[k] for k in  set(u.f.keys()).union(set(v.f.keys())) })
...
def dot(u, v):
    """
        ...
        """
    assert u.D == v.D
    return sum([u[k]*v[k] for k in u.f])
...
class Vec:
    ...
    def __str__(v):
        ...
        numdec = 3
        wd = dict([(k, (1 + max(len(str(k)), len('{0:.{1}G}'.format(v[k], numdec))))) if isinstance(v[k],int) or isinstance(
